...
let direction = null;
let score = 0;
let highScore = (document.cookie = `0`);
let isGameOver = true;
...
function draw() {
  ...
  // Create the head (first segment)
  const snakeHead = document.createElement("div");
  snakeHead.style.left = `${snake[0].x}px`;
  snakeHead.style.top = `${snake[0].y}px`;
  snakeHead.classList.add("snake", "snake-head"); // Add head class
  ...
  for (let i = 1; i < snake.length; i++) {
    ...
    snakeBody.classList.add("snake", "snake-body"); // Add body class
...
function update() {
  ...
  if (
    snakeHead.x < 0 ||
    snakeHead.y < 0 ||
    snakeHead.x >= gameSize ||
    snakeHead.y >= gameSize ||
    snake.some(
